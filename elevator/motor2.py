...
from .tof import measure, get_floor_position
from utils.sevensegment import update_display

__all__ = ["ACCURACY", "motor_forward", "motor_backward", "motor_stop", 
           "move_to", "ramp_up", "ramp_down", "go_up", "go_down", "go_to_floor"]
...
def move_to(destination) -> None:
    ...
    :param destination: The distance in mm
...
def go_to_floor(floor_number) -> None:
    """
    Moves the elevator to a specific floor (1-4)
    This is a wrapper that converts floor number to distance
    
    :param floor_number: The floor to go to (1-4)
    :type floor_number: int
    :return: None
    """
    destination = get_floor_position(floor_number)
    
    if destination is None:
        print(f"Error: Floor {floor_number} does not exist")
        return
    
    print(f"Going to floor {floor_number} (position: {destination}mm)")
    move_to(destination)
    print(f"Arrived at floor {floor_number}")
...
def go_up(destination:int) -> None:
    ...
    :param destination: Where the elevator needs to go (in mm)
    :type destination: int
    :return: None
    """
    speed = 0
